...
extern void Configurar_ADCM0SS3(void) // no lo ocupo como tal porque no lo llamo 
...
extern void Configurar_ADCM0SS2(void){ //Setup_ADC_SS2 AIN1, AIN2, AIN3 y AIN9
    //Se va a configurar A UNA VELOCIDAD DE 1 Msps
    ...
    while ((SYSCTL->PRADC & 0x01) != 0x01); // da el tiempo a la tiva para regularice su reloj, y asi se active correctamente el adc
    // 2. Configure max sampling rate (ADCn_PC_R)
...
//    ADC0->EMUX &= (0xF<<12);  // EM3: Continuo
    ...
    // 7. Set flag on sample capture  pag 868
...
{ //Setup_ADC_SS1 AIN4, AIN5, AIN6 y AIN7
    //Se va a configurar A UNA VELOCIDAD DE 1 Msps
...
//    ADC0->EMUX &= (0xF<<12);  // EM3: Continuo
    // 6. Configure ADC chanel
    ADC0->SSMUX1  = (ADC0->SSMUX1 & 0xFFFF0000) + 4; // Use chanel 4 PD3
    ADC0->SSMUX1 |= (5<<4);                          // Use chanel 5 PD2
    ADC0->SSMUX1 |= (11<<8);                          // Use chanel 11 PB5
    //ADC0->SSMUX1 |= (7<<12);                         // Use chanel 7 PD0
    // 7. Set flag on sample capture
...
extern readADC0SS3(unsigned int * adc_data)
{
...
extern readADC0SS2(unsigned int * adc_data[]){
    int i;
    // Inicia convercion SS2
    ADC0->PSSI |= (1<<2); // EM2: Software triggle
    // Waiting for conversion done
    while ((ADC0->RIS & 0x04) != 0x04);
    *(adc_data) = ADC0->SSFIFO2 & 0xFFF;
...
extern readADC0SS1(unsigned int * adc_data[]){
    // Inicia convercion SS2
    ADC0->PSSI |= (1<<1); // EM2: Software triggle
    // Waiting for conversion done
//    while ((ADC0->RIS & 0x04) != 0x04);
    Delay_ms(10);
    *(adc_data) = ADC0->SSFIFO1 & 0xFFF;     // Read 12-bit result
    ...
    *(adc_data + 3) = ADC0->SSFIFO1 & 0xFFF;     // Read 12-bit result
    ADC0->ISC |= 0x04;  // (0<<2); // EM2: Software triggle
}